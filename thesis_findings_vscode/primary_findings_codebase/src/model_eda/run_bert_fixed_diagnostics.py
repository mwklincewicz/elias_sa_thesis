...
FIXED_THRESHOLD = 0.59
MIN_PREDICTED_LABELS = 1

BASELINE_DIR = PROJECT_ROOT / "output" / "bert_emotion_model"
BASELINE_MODEL_DIR = BASELINE_DIR / "model"
if not BASELINE_MODEL_DIR.exists():
    outer_model_dir = WORKSPACE_ROOT / "output" / "bert_emotion_model" / "model"
    if outer_model_dir.exists():
        BASELINE_MODEL_DIR = outer_model_dir
FIXED_DIR = PROJECT_ROOT / "output" / SEGMENT_NAME
DIAGNOSTICS_DIR = FIXED_DIR / "diagnostics"
...
def run_bootstrap_ci(
    ...
    plot_ci_forest(
        ...
        "BERT-Hyperparamter-Fixed metric bootstrap intervals",
    )
    ...
    plot_ci_forest(
        ...
        "BERT-Hyperparamter-Fixed per-emotion F1 bootstrap intervals",
...
def run_threshold_calibration(
    ...
    ax.axvline(FIXED_THRESHOLD, color="#333333", linestyle="--", linewidth=1.2, label="Fixed threshold")
    ax.set_title("Post hoc threshold sensitivity for fixed BERT probabilities")
...
def load_bert_classifier(label_names: list[str]):
    tokenizer = AutoTokenizer.from_pretrained(BASELINE_MODEL_DIR, local_files_only=True, use_fast=True)
    model = AutoModelForSequenceClassification.from_pretrained(BASELINE_MODEL_DIR, local_files_only=True)
...
def run_lime_explanations(
    ...
    table.scale(1, 2.2)
    ax.set_title("Representative LIME explanations for fixed BERT", pad=18)
    fig.tight_layout()
...
def evaluate_untrained_lemotif(
    ...
    tokenizer = AutoTokenizer.from_pretrained(BASELINE_MODEL_DIR, local_files_only=True, use_fast=True)
    config = AutoConfig.from_pretrained(BASELINE_MODEL_DIR, local_files_only=True)
    ...
    y_pred = apply_threshold_with_min_labels(probabilities, FIXED_THRESHOLD, MIN_PREDICTED_LABELS)
...
def plot_paired_delta(delta_frame: pd.DataFrame, path: Path, title: str) -> None:
    ...
    ax.set_xlabel("Delta: fixed BERT minus comparison")
    ax.set_title(title)
    fig.tight_layout()
    save_figure(fig, path)
...
def run_lemotif_comparisons(
    ...
    if gemma_metrics_path is None or gemma_predictions_path is None:
        ...
        model_metrics = {
            ...
            "BERT-Hyperparamter-Fixed": fixed_metrics,
        }
        model_predictions = {
            "Untrained BERT": untrained_predictions,
            "BERT-Hyperparamter-Fixed": fixed_predictions,
        }
    else:
    ...
    comparisons = {name: preds for name, preds in model_predictions.items() if name != "BERT-Hyperparamter-Fixed"}
...
def run_stories_comparisons() -> None:
    trained_metrics_path = first_existing(
        [
            PROJECT_ROOT / "test data" / "output" / "external_evaluation" / "metrics.json",
            WORKSPACE_ROOT / "test data" / "output" / "external_evaluation" / "metrics.json",
        ]
    )
    ...
    if trained_metrics_path is None or untrained_metrics_path is None:
        (COMPARATIVE_DIR / "stories_missing.txt").write_text(
            "Stories trained/untrained BERT metrics were not found, so Stories comparisons were skipped.\n",
            encoding="utf-8",
        )
        return
    ...
    emotion_rows = []
    for model_name, metrics in [("Trained BERT", trained), ("Untrained BERT", untrained)]:
        emotion = metrics["test_metrics"]
        emotion_rows.extend(
            [
    ...
    ax.set_title("Stories emotion comparison: trained vs untrained BERT")
    ax.set_xlabel("")
    ax.set_ylabel("Score")
    ax.tick_params(axis="x", rotation=16)
    ax.legend(title="")
...
def write_summary(args: argparse.Namespace) -> None:
    lines = [
        "# BERT-Hyperparamter-Fixed Diagnostics",
        "",
        "Generated sections:",
        "- 01_bootstrap_ci: fixed-test bootstrap intervals.",
        ...
        "- 06_lime: BERT LIME explanations." if not args.skip_lime else "- 06_lime: skipped by --skip-lime.",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate diagnostics and comparisons for BERT-Hyperparamter-Fixed."
    )
    parser.add_argument(
        "--skip-lime",
        action="store_true",
    ...
    parser.add_argument(
        "--only-lime",
        action="store_true",
        help="Only regenerate the LIME explanation section from existing fixed predictions.",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    if not FIXED_METRICS_PATH.exists() or not FIXED_PREDICTIONS_PATH.exists():
        raise SystemExit(
            "BERT-Hyperparamter-Fixed artifacts are missing. "
            "Run src/model_eda/build_bert_hyperparameter_fixed.py first."
        )
    if not BASELINE_MODEL_DIR.exists():
        raise SystemExit(
            "The trained BERT checkpoint is missing at output/bert_emotion_model/model. "
            "LIME and untrained comparison need the saved tokenizer/config."
        )

    fixed_metrics = read_json(FIXED_METRICS_PATH)
    ...
    if args.only_lime:
        ...
        write_summary(args)
        print(f"Saved BERT-Hyperparamter-Fixed LIME diagnostics to: {LIME_DIR}")
        return
    ...
    write_summary(args)
    print(f"Saved BERT-Hyperparamter-Fixed diagnostics to: {DIAGNOSTICS_DIR}")
